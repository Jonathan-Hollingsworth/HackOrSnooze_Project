...
class User {
  ...
  /**Marks a given story as one of the user's favorites and updates the API*/
  async markStoryAsFavorite(storyId) {
    console.debug("markStoryAsFavorite")
    await axios({
  ...
  /**Unlists a given favorite story and updates the API*/
  async unmarkStoryAsFavorite(storyId) {
    console.debug("unmarkStoryAsFavorite")
    await axios({
      url: `https://hack-or-snooze-v3.herokuapp.com/users/${this.username}/favorites/${storyId}`,
      method: "DELETE",
      data: {token: this.loginToken}
